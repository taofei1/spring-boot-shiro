...
import com.neo.guava.ScanPackage;
...
public class GenUtils
{
    ...
    public static VelocityContext getVelocityContext(TableInfo table)
    {
        ...
        velocityContext.put("author", GenerateCodeSettings.AUTHOR);
    ...
    {
        if (GenerateCodeSettings.REMOVE_PREFIX)
        {
            tableName = tableName.substring(tableName.indexOf("_") + 1);
        }
        if (StringUtils.isNotEmpty(GenerateCodeSettings.TABLE_PREFIX))
        {
            tableName = tableName.replace(GenerateCodeSettings.TABLE_PREFIX, "");
        }
    ...
    public static String fieldToColumn(String field){
        ...
        for(int i=0;i<f.length;i++){
            if(Character.isUpperCase(f[i])){
               sb.append("_");
            }
            sb.append(Character.toLowerCase(f[i]));
        }
        return sb.toString();
    ...
    public static String columnToField(String column){
        ...
        StringBuffer sb=new StringBuffer(list.get(0).toLowerCase());
    ...
    //扫描包下的类,是否递归子目录速度极慢
    public static List<String> scanPackage(String packageName,boolean recursion) throws IOException {
        ClassPath classpath = ClassPath.from(GenUtils.class.getClassLoader());
        ImmutableSet<ClassPath.ClassInfo> topLevelClasses;
        if(recursion){
            topLevelClasses=classpath.getTopLevelClassesRecursive(packageName);
        }else{
            topLevelClasses = classpath.getTopLevelClasses(packageName);
        }
        List<String> list=new ArrayList<>();
        topLevelClasses.forEach((c)->list.add(c.getName()));
        return list;
