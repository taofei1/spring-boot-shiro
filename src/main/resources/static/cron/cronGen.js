(function ($) {
    ...
    $.fn.extend({
        cronGen: function (options) {
            ...
            // Add an input
            var $i = $("<input>", { type: 'text', readonly:'readonly',id:'cronExpression',name:'cronExpression'}).addClass("form-control").val($(that).val());
            $i.appendTo($g);
            // Add the button
            var $b = $("<button class=\"btn btn-default\"><i class=\"glyphicon  glyphicon-edit\"></i></button>");
            ...
            $b.popover({
                ...
                $("#chCron").text(cronParse(inputElement.val()));
                //绑定指定事件
    ...
    var generate = function () {
        ...
        var results = "";
        console.log($("input:radio[name=second]:checked")[0]);
        switch (activeTab) {
            case "SecondlyTab":
    ...
    $.fn.cronGen.tools = {
        ...
        cronParse : function(cronExpress) {
            ...
            if (cronExpress) {
                ...
                console.log(regs);
                $("input[id=secondHidden]").val(regs[0]);
                $("input[id=minHidden]").val(regs[1]);
                $("input[id=hourHidden]").val(regs[2]);
                $("input[id=dayHidden]").val(regs[3]);
                $("input[id=monthHidden]").val(regs[4]);
                $("input[id=weekHidden]").val(regs[5]);
